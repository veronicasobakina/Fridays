...
import com.example.friday.Utils.AndroidUtil;
...
public class LoginOtpActivity extends AppCompatActivity {
                ...
                PhoneAuthOptions.newBuilder(mAuth)
                        ...
                        .setCallbacks(new PhoneAuthProvider.OnVerificationStateChangedCallbacks() {
                            ...
                            public void onVerificationFailed(@NonNull FirebaseException e) {
                                AndroidUtil.showToast(getApplicationContext(),"Не удалось выполнить проверку кода");
                                setInProgress(false);
                            }
                            ...
                            public void onCodeSent(@NonNull String s, @NonNull PhoneAuthProvider.ForceResendingToken forceResendingToken) {
                                ...
                                AndroidUtil.showToast(getApplicationContext(),"Код отправлен успешно");
                                setInProgress(false);
                            }
                        });
        if(isResend){
    ...
    void startResendTimer(){
        ...
        timer.scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                timeoutSeconds--;
                resendOtpTextView.setText("Новый код через "+timeoutSeconds +" сек");
                if(timeoutSeconds<=0){
                    timeoutSeconds =60L;
                    timer.cancel();
                    runOnUiThread(() -> {
